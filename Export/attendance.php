...
if($from!=="" && $to!=="") // If date range is selected
{
...
else // If date range is not selected
{
   $msg = "ATTENDANCE REPORT OF ".strtoupper($employee_name)."";
...
if($rows>0)
{
   ...
   while($data=mysqli_fetch_assoc($result))
   {
      ...
      <td style='text-align:center;'>".date('F-d-Y',strtotime($data['date_created']))."</td>
      <td style='text-align:center;'>".$data['reason']."</td>
      <td style='text-align:center;'>".$data['marked_by']."</td>
      </tr>";
   }
}
else
